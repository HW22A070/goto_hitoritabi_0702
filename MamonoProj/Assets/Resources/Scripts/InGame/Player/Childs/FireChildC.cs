...
public class FireChildC : ChildsCoreC
{
    ...
    void Update()
    {
        ...
        _posPlayer = _playerGO.transform.position +new Vector3(_scPlayer.CheckPlayerAngleIsRight() ? -24 : 24, Mathf.Cos(_radius * Mathf.Deg2Rad)*4, 0);
        _posDelta = Moving2DSystems.GetSneaking(_pos, _posPlayer, 3);
    }
    ...
    public void DoAttackRocket()
    {
        ...
        Instantiate(_prfbBomb, _pos, transform.rotation).ShotHowitzer((_scPlayer.CheckPlayerAngleIsRight() ? 0 : 180) + Random.Range(-5, 5), 20, -0.3f, 660, 20, 1.0f);
    }
