...
public class PFireBallC : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_expMode == 0)
        {
            _posDelta = Moving2DSystems.GetSneaking(_pos, _firstPos, 20);
        ...
        else if(_expMode==1)
        {
            ...
            if (_goTarget != null)
            {
                _posDelta = Moving2DSystems.GetDirection(Moving2DSystems.GetAngle(_pos, _goTarget.transform.position))*16;
            }
            else
            {
                EXPEffect();
    ...
    private IEnumerator Explosion()
    {
        ...
        for (int j = 0; j < 10; j++)
        {
            for (int k = 0; k < 2; k++)
            {
                ...
                Instantiate(ExpPrefab, _pos, transform.localRotation).ShotEXP(Random.Range(0, 360)/*((360 / hunj) * j)+(k*180)*/, 10, 0.5f);
            }
